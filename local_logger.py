...
class MyLocalLogger:
    ...
    def log(self, key, value, epoch: int):
        ...
        # handle the ema_val_dice special case! It is automatically logged when we add a new val_dice
        if key == 'val_dice':
            new_ema_pseudo_dice = self.get_value('val_dice', step=epoch-1) * 0.9 + 0.1 * value \
                if len(self.get_value('val_dice', step=None)) > 0 else value
            self.log('ema_val_dice', new_ema_pseudo_dice, epoch)
    ...
    def plot_progress_png(self, output_folder):
        ...
        ax.plot(x_values, self.my_fantastic_logging['train_loss'][:epoch], color='b', ls='-', label="loss_tr", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['val_loss'][:epoch], color='r', ls='-', label="loss_val", linewidth=4)
        ax2.plot(x_values, self.my_fantastic_logging['val_dice'][:epoch], color='g', ls='dotted', label="pseudo dice",
                 linewidth=3)
        ax2.plot(x_values, self.my_fantastic_logging['ema_val_dice'][:epoch], color='g', ls='-', label="pseudo dice (mov. avg.)",
                 linewidth=4)
        ax.set_xlabel("epoch")
        ax.set_ylabel("loss")
        ...
        ax = ax_all[1]
        ax.plot(x_values, [i - j for i, j in zip(self.my_fantastic_logging['epoch_end_timestamps'][:epoch],
                                                 self.my_fantastic_logging['epoch_start_timestamps'])][:epoch], color='b',
        ...
        # learning rate
        ax = ax_all[2]
        ax.plot(x_values, self.my_fantastic_logging['lrs'][:epoch], color='b', ls='-', label="learning rate", linewidth=4)
        ax.set_xlabel("epoch")
